(function () {
    ...
    var log = {
        ...
        info: function () {
            if (this._ok('info')) console.log.apply(console, ['[Rutor]'].concat(Array.prototype.slice.call(arguments)));
        },
        debug: function () {
            if (this._ok('debug')) console.log.apply(console, ['[Rutor:debug]'].concat(Array.prototype.slice.call(arguments)));
        },
        error: function () {
            if (this._ok('error')) console.error.apply(console, ['[Rutor:error]'].concat(Array.prototype.slice.call(arguments)));
        },
    ...
    function cleanTitle(name) {
        ...
        if (!titlePart) titlePart = name.split(/[\(\[]/)[0].split('/')[0].trim();
        return titlePart;
    }
    ...
    function fetchHtml(url, success, error) {
        ...
        network.silent(finalUrl, function (html) {
            if (typeof html === 'string' && html.length > 300) success(html);
            else {
    ...
    function searchTMDB(torrentName, callback) {
        ...
        var apiUrl = Lampa.TMDB.api(endpoint + '?query=' + q + (year ? '&' + yearParam + '=' + year : '') + '&language=' + lang + '&api_key=' + Lampa.TMDB.key());
    ...
    function parseTorrents(html, limit) {
        ...
        while ((m = rowRegex.exec(html)) !== null && i < limit) {
            ...
            if (!titleM) continue;
            var comments = (row.match(/alt="C"[^>]*>[\s\u00a0]*(\d+)/) || row.match(/<\/a>\s*(\d+)\s*<\/td>/) || [])[1] || '0';
            list.push({
                title: titleM[2].trim(),
                url: BASE + titleM[1],
                comments: comments,
    ...
    // Minimal table item (native-style)
    function createTableItem(t) {
        var item = $('<div class="torrent-item selector" style="display:flex;align-items:center;padding:0.6em 1em;border-bottom:1px solid rgba(255,255,255,0.08);">');
        item.append($('<div style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;padding-right:1em;">').text(t.title));
        var meta = $('<div style="display:flex;gap:1.2em;flex-shrink:0;font-size:0.95em;">');
        meta.append($('<span style="opacity:0.7;">').text(t.comments || '0'));
        meta.append($('<span style="color:#4caf50;">').html('↑ ' + (t.seeds || '0')));
        meta.append($('<span style="color:#f44336;">').html('↓ ' + (t.leeches || '0')));
        item.append(meta);
        item.on('hover:enter', function () {
            // use existing torrserver → player path
            if (window.TorrServer && typeof window.TorrServer.open === 'function') {
                window.TorrServer.open(t.url);
            } else if (Lampa.Torrent && Lampa.Torrent.open) {
                Lampa.Torrent.open({ title: t.title, url: t.url, magnet: t.url });
            } else {
    ...
    var Api = {
        ...
        category: function (params, onSuccess, onError) {
            ...
            fetchHtml(BASE + '/top', function (html) {
                ...
                if (getViewMode() === 'table') {
                    // minimal table mode for top
                    var results = cats.map(function (cat) {
                        return {
                            title: cat.title,
                            results: cat.torrents.map(function (t) {
                                return {
                                    title: t.title,
                                    rutor: t,
                                    source: SOURCE,
                    ...
                    onSuccess({ results: results, page: 1, total_pages: 1, source: SOURCE, view: 'table' });
                    return;
                }
        ...
        list: function (params, onSuccess, onError) {
            ...
            fetchHtml(url, function (html) {
                ...
                if (getViewMode() === 'table') {
                    onSuccess({
                        results: torrents.map(function (t) {
                            return {
                                title: t.title,
                                rutor: t,
                                source: SOURCE,
    ...
    function addSettings() {
        ...
        Lampa.SettingsApi.addParam({
            ...
            onChange: function () { Lampa.Noty.show('Прокси сохранён'); }
        });
        ...
        Lampa.SettingsApi.addParam({
            ...
            param: {
                name: 'rutor_limit',
                type: 'select',
                values: { '5': '5', '8': '8', '10': '10', '12': '12', '15': '15' },
                default: '10'
            },
            field: {
                name: 'Торрентов в ряду (топ)',
                description: 'Сколько раздач в каждой категории на главной (макс. 15)'
            }
        });
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                ...
                description: 'Сколько раздач загружать при открытии категории (20–100)'
            }
        });
        ...
        Lampa.SettingsApi.addParam({
            ...
            param: {
                ...
                values: {
                    ...
                    'table': 'Таблица (как rutor)'
                },
                default: 'cards'
            },
            field: {
                name: 'Режим отображения',
                description: 'Карточки — нативные карточки Lampa. Таблица — список как на сайте rutor'
            },
            onChange: function () { Lampa.Noty.show('Режим сохранён. Перезайдите в Rutor'); }
        });

        Lampa.SettingsApi.addParam({
            component: 'rutor_top',
            param: { name: 'rutor_cache', type: 'trigger', default: true },
            field: { name: 'Кеш TMDB', description: 'Запоминать найденные карточки (7 дней)' }
        });
        ...
        Lampa.SettingsApi.addParam({
            ...
            param: {
                name: 'rutor_log',
                type: 'select',
                values: { 'off': 'Выкл', 'error': 'Только ошибки', 'info': 'Инфо', 'debug': 'Отладка' },
                default: 'info'
            },
            field: { name: 'Логирование', description: 'Уровень логов в консоли' }
        });

        Lampa.SettingsApi.addParam({
            component: 'rutor_top',
            param: { name: 'rutor_clear_cache', type: 'trigger', default: false },
            field: { name: 'Очистить кеш TMDB', description: 'Удалить сохранённые соответствия' },
            onChange: function (v) {
                if (v === true || v === 'true') {
                    Lampa.Modal.open({
                        title: 'Очистить кеш?',
                        ...
                        buttons: [
                            { name: 'Нет', onSelect: function () { Lampa.Modal.close(); Lampa.Storage.set('rutor_clear_cache', false); Lampa.Controller.toggle('settings'); } },
                            { name: 'Да', onSelect: function () {
                                Lampa.Modal.close();
                                cache = {};
                        ...
                        onBack: function () { Lampa.Modal.close(); Lampa.Storage.set('rutor_clear_cache', false); Lampa.Controller.toggle('settings'); }
                    });
                }
            }
        });
