(function () {
	...
	function rating_kp_imdb(card) {
		...
		function _showRating(data) {
			if (data) {
        var showKp = Lampa.Storage.get('rating_show_kp', true);
        var showImdb = Lampa.Storage.get('rating_show_imdb', true);
        if (showImdb) $('.rate--imdb', render).removeClass('hide').find('> div').eq(0).text(imdb_rating);
        else $('.rate--imdb', render).addClass('hide');
        if (showKp) $('.rate--kp', render).removeClass('hide').find('> div').eq(0).text(kp_rating);
        else $('.rate--kp', render).addClass('hide');
			}
		}
	}
    ...
    field: {
        name: 'Показывать KP',
        description: 'Отображать рейтинг Кинопоиска'
    }
    });
    Lampa.SettingsApi.addParam({
        component: 'player',
        ...
        field: {
            ...
            description: 'Отображать рейтинг IMDb'
        }
    });
	}
